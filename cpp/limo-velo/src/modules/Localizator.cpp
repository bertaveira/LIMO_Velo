...
void Localizator::initialize(double t, IMUs & imus)
{
    this->initial_time = t;
...
void Localizator::calculate_H(
    const state_ikfom & s, const Matches & matches, Eigen::MatrixXd & H,
    Eigen::VectorXd & h)
{
...
void Localizator::propagate_to(IMUs imus, double t)
{
    // Get new IMUs
    ...
    // TODO: [Bernardo] check if this causes issues due to the last IMU being propagated partially)
...
std::shared_ptr<Localizator> LocalizatorInstance = std::make_shared<Localizator>();
