...
bool R3Math::is_plane(
    const Eigen::Matrix<float, 4, 1> & pca_result, const PointVector & point,
    const float & threshold)
{
    for (int j = 0; j < point.size(); j++) {
        float res = pca_result(0) * point[j].x + pca_result(1) * point[j].y + pca_result(2) *
          point[j].z + pca_result(3);
