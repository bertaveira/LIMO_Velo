...
void Mapper::add(Points & points, double time, bool downsample)
{
    ...
    if (not this->exists()) {this->build_tree(points);} else {this->add_points(points, downsample);}
...
{
    PointVector as_vector; for (Point p : points) {
        as_vector.push_back(p);
    }
...
std::shared_ptr<Mapper> MapperInstance = std::make_shared<Mapper>();
