...
public:
    ...
    Publishers(rclcpp::Node::SharedPtr node)
    {
        ...
        this->states_pub = node->create_publisher<geometry_msgs::msg::PoseArray>(
            "/limovelo/states",
            1000);
        ...
        this->pcl_pub =
          node->create_publisher<sensor_msgs::msg::PointCloud2>("/limovelo/pcl", 1000);
        this->full_pcl_pub = node->create_publisher<sensor_msgs::msg::PointCloud2>(
            "/limovelo/full_pcl", 1000);

        this->gt_pub = node->create_publisher<nav_msgs::msg::Odometry>("/limovelo/gt", 1000);

        this->planes_pub = node->create_publisher<geometry_msgs::msg::PoseArray>(
            "/limovelo/planes",
            1000);
    ...
    void pointcloud(Points & points, bool part = false)
    {
        ...
        if (part) {this->publish_pcl(pcl, this->pcl_pub);} else {
            this->publish_pcl(pcl, this->full_pcl_pub);
        }
    }
    ...
    void t1_t2(
        const Points & points, const IMUs & imus, const States & states, double t1,
        double t2)
    {
...
private:
    ...
    void publish_planes(const Planes & planes)
    {
        ...
        if (this->planes_pub->get_subscription_count() > 0) {
    ...
    void publish_pcl(
        const pcl::PointCloud<full_info::Point> & pcl,
        ...
        double t2)
    {
        if (points.size() > 0 and imus.size() > 0 and states.size() > 0) {
            ...
            std::cout << std::setprecision(16) << "L: " << points.front().time << " -- " <<
                points.back().time << " = " << points.size() << std::endl;
            std::cout << std::setprecision(16) << "I: " << imus.front().time << " -- " <<
                imus.back().time << " = " << imus.size() << std::endl;
            std::cout << std::setprecision(16) << "X: " << states.front().time << " -- " <<
                states.back().time << " = " << states.size() << std::endl;
        }
    }
